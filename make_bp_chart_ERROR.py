...
table_extra = 80 + (len(rows) + 1) * 24
W, H = 1100, 650 + table_extra

#W, H = 1100, 1050
m_left, m_right, m_top = 90, 40, 70
chart_bottom = 330
m_bottom = chart_bottom
pw, ph = W - m_left - m_right, 650 - m_top - m_bottom
...
table_x = m_left
table_y = H - 300
table_w = W - m_left - m_right
row_h = 24

headers = ["Date", "Time", "Systolic", "Diastolic", "Pulse"]
col_w = [180, 140, 140, 140, 120]

n_rows = len(rows) + 1
table_h = row_h * n_rows

# table title
parts.append(f'<text x="{table_x}" y="{table_y - 12}" font-size="16" font-family="Arial" font-weight="bold">Blood Pressure Data</text>')

# outer border
parts.append(f'<rect x="{table_x}" y="{table_y}" width="{table_w}" height="{table_h}" fill="white" stroke="#cfcfcf"/>')

# header background
parts.append(f'<rect x="{table_x}" y="{table_y}" width="{table_w}" height="{row_h}" fill="#f3f4f6" stroke="none"/>')

# vertical lines and header text
x = table_x
for i, h in enumerate(headers):
    parts.append(f'<text x="{x + 8}" y="{table_y + 16}" font-size="12" font-family="Arial" font-weight="bold" fill="#111">{h}</text>')
    x += col_w[i]
    if i < len(col_w) - 1:
        parts.append(f'<line x1="{x}" y1="{table_y}" x2="{x}" y2="{table_y + table_h}" stroke="#d1d5db" stroke-width="1"/>')

# horizontal lines
for i in range(1, n_rows):
    y = table_y + i * row_h
    parts.append(f'<line x1="{table_x}" y1="{y}" x2="{table_x + table_w}" y2="{y}" stroke="#e5e7eb" stroke-width="1"/>')


for idx, (dt, sys, dia, pulse) in enumerate(rows, start=1):
    ...
    for i, val in enumerate(values):
        parts.append(f'<text x="{x + 8}" y="{y}" font-size="12" font-family="Arial" fill="#222">{val}</text>')
        x += col_w[i]
parts.append('</svg>')
